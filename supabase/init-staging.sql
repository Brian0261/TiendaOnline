-- ==============================================================================
-- SCRIPT DE INICIALIZACION STAGING (PARIDAD CON SEEDS DE PRODUCCION DEL REPO)
-- Base: 03_schema.sql + delivery_v2 + 04_seed_core.sql + 05_seed_demo.sql
-- ==============================================================================

--- [SECCION] ESQUEMA BASE (03_schema.sql) ---

-- PostgreSQL schema

DROP TABLE IF EXISTS payment_webhook_event CASCADE;
DROP TABLE IF EXISTS payment_intent CASCADE;
DROP TABLE IF EXISTS detalle_devolucion CASCADE;
DROP TABLE IF EXISTS devolucion CASCADE;
DROP TABLE IF EXISTS historial CASCADE;
DROP TABLE IF EXISTS pedido_inventario_mov CASCADE;
DROP TABLE IF EXISTS reserva_stock_item CASCADE;
DROP TABLE IF EXISTS reserva_stock CASCADE;
DROP TABLE IF EXISTS detalle_pedido CASCADE;
DROP TABLE IF EXISTS boleta CASCADE;
DROP TABLE IF EXISTS factura CASCADE;
DROP TABLE IF EXISTS comprobante CASCADE;
DROP TABLE IF EXISTS envio CASCADE;
DROP TABLE IF EXISTS pedido CASCADE;
DROP TABLE IF EXISTS salida_inventario CASCADE;
DROP TABLE IF EXISTS entrada_inventario CASCADE;
DROP TABLE IF EXISTS proveedor_producto CASCADE;
DROP TABLE IF EXISTS inventario CASCADE;
DROP TABLE IF EXISTS motorizado CASCADE;
DROP TABLE IF EXISTS delivery CASCADE;
DROP TABLE IF EXISTS almacen CASCADE;
DROP TABLE IF EXISTS proveedor CASCADE;
DROP TABLE IF EXISTS carrito CASCADE;
DROP TABLE IF EXISTS producto CASCADE;
DROP TABLE IF EXISTS marca CASCADE;
DROP TABLE IF EXISTS categoria CASCADE;
DROP TABLE IF EXISTS metodos_de_pago CASCADE;
DROP TABLE IF EXISTS reclamo CASCADE;
DROP TABLE IF EXISTS password_reset_token CASCADE;
DROP TABLE IF EXISTS email_verification_token CASCADE;
DROP TABLE IF EXISTS usuario CASCADE;

CREATE TABLE usuario (
    id_usuario INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    nombre VARCHAR(50) NOT NULL,
    apellido VARCHAR(50) NOT NULL,
    email VARCHAR(100) NOT NULL UNIQUE,
    contrasena VARCHAR(255) NOT NULL,
    fecha_registro TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    telefono VARCHAR(20),
    direccion_principal VARCHAR(255),
    rol VARCHAR(15) NOT NULL DEFAULT 'CLIENTE' CHECK (rol IN ('CLIENTE', 'ADMINISTRADOR', 'EMPLEADO', 'REPARTIDOR')),
    email_verificado BOOLEAN NOT NULL DEFAULT FALSE,
    email_verificado_en TIMESTAMPTZ NULL
);

CREATE TABLE email_verification_token (
    id_email_verification_token INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    id_usuario INT NOT NULL REFERENCES usuario(id_usuario),
    token_hash VARCHAR(64) NOT NULL,
    expires_at TIMESTAMPTZ NOT NULL,
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    used_at TIMESTAMPTZ NULL,
    CONSTRAINT uq_emailverificationtoken UNIQUE (token_hash)
);

CREATE TABLE password_reset_token (
    id_password_reset_token INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    id_usuario INT NOT NULL REFERENCES usuario(id_usuario),
    token_hash VARCHAR(64) NOT NULL,
    expires_at TIMESTAMPTZ NOT NULL,
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    used_at TIMESTAMPTZ NULL,
    CONSTRAINT uq_passwordresettoken UNIQUE (token_hash)
);

CREATE TABLE metodos_de_pago (
    id_metodo_pago INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    tipo_metodo VARCHAR(50) NOT NULL,
    detalles VARCHAR(255)
);

CREATE TABLE categoria (
    id_categoria INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    nombre_categoria VARCHAR(100) NOT NULL UNIQUE
);

CREATE TABLE marca (
    id_marca INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    nombre_marca VARCHAR(100) NOT NULL UNIQUE
);

CREATE TABLE proveedor (
    id_proveedor INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    nombre_proveedor VARCHAR(100) NOT NULL,
    telefono VARCHAR(20) NOT NULL,
    email VARCHAR(100),
    direccion VARCHAR(255)
);

CREATE TABLE almacen (
    id_almacen INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    nombre_almacen VARCHAR(100) NOT NULL,
    direccion VARCHAR(255) NOT NULL,
    telefono VARCHAR(20) NOT NULL,
    responsable VARCHAR(100) NOT NULL
);

CREATE TABLE delivery (
    id_delivery INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    nombre_empresa VARCHAR(100) NOT NULL,
    telefono VARCHAR(20) NOT NULL,
    email VARCHAR(100)
);

CREATE TABLE motorizado (
    id_motorizado INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    nombre VARCHAR(100) NOT NULL,
    apellido VARCHAR(100) NOT NULL,
    telefono VARCHAR(20) NOT NULL,
    licencia VARCHAR(50) NOT NULL,
    id_delivery INT NOT NULL REFERENCES delivery(id_delivery),
    id_usuario INT NULL UNIQUE REFERENCES usuario(id_usuario)
);

CREATE TABLE producto (
    id_producto INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    nombre_producto VARCHAR(100) NOT NULL,
    descripcion TEXT,
    precio DECIMAL(10,2) NOT NULL,
    imagen VARCHAR(255) NOT NULL DEFAULT '/assets/images/placeholder-product.png',
    activo BOOLEAN NOT NULL DEFAULT TRUE,
    id_categoria INT NOT NULL REFERENCES categoria(id_categoria),
    id_marca INT NOT NULL REFERENCES marca(id_marca)
);

CREATE TABLE proveedor_producto (
    id_proveedor_producto INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    precio_unitario DECIMAL(10,2) NOT NULL,
    precio_costo DECIMAL(10,2) NOT NULL,
    fecha_ultima_compra TIMESTAMPTZ,
    id_producto INT NOT NULL REFERENCES producto(id_producto),
    id_proveedor INT NOT NULL REFERENCES proveedor(id_proveedor)
);

CREATE TABLE inventario (
    id_inventario INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    cantidad_disponible INT NOT NULL,
    fecha_ultima_actualizacion TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    id_producto INT NOT NULL REFERENCES producto(id_producto),
    id_almacen INT NOT NULL REFERENCES almacen(id_almacen)
);

CREATE TABLE entrada_inventario (
    id_entrada_inventario INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    cantidad_recibida INT NOT NULL,
    fecha_entrada TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    motivo_entrada VARCHAR(255) NOT NULL,
    id_inventario INT NOT NULL REFERENCES inventario(id_inventario)
);

CREATE TABLE salida_inventario (
    id_salida_inventario INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    cantidad_salida INT NOT NULL,
    fecha_salida TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    motivo_salida VARCHAR(255) NOT NULL,
    id_inventario INT NOT NULL REFERENCES inventario(id_inventario),
    id_usuario INT NULL REFERENCES usuario(id_usuario)
);

CREATE TABLE pedido (
    id_pedido INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    fecha_creacion TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    estado_pedido VARCHAR(50) NOT NULL
        CHECK (estado_pedido IN ('PENDIENTE_PAGO', 'PENDIENTE', 'PREPARADO', 'EN CAMINO', 'ENTREGADO', 'CANCELADO', 'REEMBOLSADO'))
        DEFAULT 'PENDIENTE_PAGO',
    total_pedido DECIMAL(10,2) NOT NULL,
    costo_envio DECIMAL(10,2),
    direccion_envio VARCHAR(255) NOT NULL,
    id_usuario INT NOT NULL REFERENCES usuario(id_usuario),
    id_metodo_pago INT NOT NULL REFERENCES metodos_de_pago(id_metodo_pago)
);

CREATE TABLE reserva_stock (
    id_reserva_stock INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    id_pedido INT NOT NULL REFERENCES pedido(id_pedido),
    id_usuario INT NOT NULL REFERENCES usuario(id_usuario),
    estado VARCHAR(20) NOT NULL CHECK (estado IN ('ACTIVA', 'CONSUMIDA', 'LIBERADA', 'EXPIRADA')),
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    expires_at TIMESTAMPTZ NOT NULL
);

CREATE TABLE reserva_stock_item (
    id_reserva_stock_item INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    id_reserva_stock INT NOT NULL REFERENCES reserva_stock(id_reserva_stock),
    id_producto INT NOT NULL REFERENCES producto(id_producto),
    cantidad INT NOT NULL CHECK (cantidad > 0),
    CONSTRAINT uq_reservastockitem UNIQUE (id_reserva_stock, id_producto)
);

CREATE TABLE pedido_inventario_mov (
    id_pedido_inventario_mov INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    id_pedido INT NOT NULL REFERENCES pedido(id_pedido),
    id_inventario INT NOT NULL REFERENCES inventario(id_inventario),
    tipo_mov VARCHAR(10) NOT NULL CHECK (tipo_mov IN ('SALIDA','ENTRADA')),
    cantidad INT NOT NULL CHECK (cantidad > 0),
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

CREATE TABLE envio (
    id_envio INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    numero_rastreo VARCHAR(50) NOT NULL,
    transportista VARCHAR(100) NOT NULL,
    fecha_envio TIMESTAMPTZ,
    estado_envio VARCHAR(50) NOT NULL,
    costo_envio DECIMAL(10,2) NOT NULL,
    id_pedido INT NOT NULL REFERENCES pedido(id_pedido),
    id_delivery INT REFERENCES delivery(id_delivery)
);

CREATE TABLE comprobante (
    id_comprobante INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    tipo_comprobante VARCHAR(50) NOT NULL,
    numero_comprobante VARCHAR(50) NOT NULL UNIQUE,
    fecha_creacion TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    monto_total DECIMAL(10,2) NOT NULL,
    estado_comprobante VARCHAR(50) NOT NULL CHECK (estado_comprobante IN ('PENDIENTE','PAGADO','ANULADO','OBSERVADO')),
    fecha_emision TIMESTAMPTZ NULL,
    ultima_actualizacion TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    id_pedido INT NOT NULL REFERENCES pedido(id_pedido),
    id_metodo_pago INT NOT NULL REFERENCES metodos_de_pago(id_metodo_pago),
    CONSTRAINT ck_comprobante_fechaemision
    CHECK (
        (estado_comprobante = 'PENDIENTE' AND fecha_emision IS NULL) OR
        (estado_comprobante <> 'PENDIENTE' AND fecha_emision IS NOT NULL)
    )
);

CREATE TABLE boleta (
    id_boleta INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    nombre_cliente VARCHAR(100) NOT NULL,
    dni_cliente VARCHAR(20) NOT NULL,
    id_comprobante INT NOT NULL REFERENCES comprobante(id_comprobante)
);

CREATE TABLE factura (
    id_factura INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    monto_base DECIMAL(10,2) NOT NULL,
    monto_impuesto DECIMAL(10,2) NOT NULL,
    razon_social_cliente VARCHAR(100) NOT NULL,
    ruc_cliente VARCHAR(20) NOT NULL,
    direccion_cliente VARCHAR(255),
    id_comprobante INT NOT NULL REFERENCES comprobante(id_comprobante)
);

CREATE TABLE detalle_pedido (
    id_detalle_pedido INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    cantidad INT NOT NULL,
    precio_unitario_venta DECIMAL(10,2) NOT NULL,
    subtotal DECIMAL(10,2) NOT NULL,
    id_pedido INT NOT NULL REFERENCES pedido(id_pedido),
    id_producto INT NOT NULL REFERENCES producto(id_producto)
);

CREATE TABLE devolucion (
    id_devolucion INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    fecha_solicitud TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    estado_devolucion VARCHAR(50) NOT NULL,
    razon_devolucion TEXT NOT NULL,
    id_pedido INT NOT NULL REFERENCES pedido(id_pedido),
    id_usuario INT NOT NULL REFERENCES usuario(id_usuario)
);

CREATE TABLE detalle_devolucion (
    id_detalle_devolucion INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    cantidad_devuelta INT NOT NULL,
    condicion_producto VARCHAR(50) NOT NULL,
    precio_unitario_devolucion DECIMAL(10,2) NOT NULL,
    id_devolucion INT NOT NULL REFERENCES devolucion(id_devolucion),
    id_detalle_pedido INT NOT NULL REFERENCES detalle_pedido(id_detalle_pedido)
);

CREATE TABLE reclamo (
    id_reclamo INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    tipo_reclamo VARCHAR(100) NOT NULL,
    descripcion_problema TEXT NOT NULL,
    estado_reclamo VARCHAR(50) NOT NULL,
    fecha_creacion TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    id_usuario INT NOT NULL REFERENCES usuario(id_usuario)
);

CREATE TABLE historial (
    id_historial INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    descripcion TEXT NOT NULL,
    accion VARCHAR(100) NOT NULL,
    fecha_accion TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    id_reclamo INT REFERENCES reclamo(id_reclamo),
    id_pedido INT REFERENCES pedido(id_pedido),
    id_usuario INT NOT NULL REFERENCES usuario(id_usuario)
);

CREATE TABLE carrito (
    id_carrito  INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    id_usuario  INT NOT NULL REFERENCES usuario(id_usuario),
    id_producto INT NOT NULL REFERENCES producto(id_producto),
    cantidad    INT NOT NULL CHECK (cantidad > 0),
    CONSTRAINT uq_carrito UNIQUE (id_usuario, id_producto)
);

--- [SECCION] DELIVERY V2 (migration) ---

-- Migration: delivery v2
-- Date: 2026-03-03
-- Source: apps/api/db/init/08_delivery_v2.sql

-- 1) Permitir rol REPARTIDOR en instalaciones existentes
ALTER TABLE usuario DROP CONSTRAINT IF EXISTS usuario_rol_check;
ALTER TABLE usuario
  ADD CONSTRAINT usuario_rol_check
  CHECK (rol IN ('CLIENTE', 'ADMINISTRADOR', 'EMPLEADO', 'REPARTIDOR'));

-- 2) Vincular motorizado con usuario del sistema
ALTER TABLE motorizado
  ADD COLUMN IF NOT EXISTS id_usuario INT NULL UNIQUE REFERENCES usuario(id_usuario);

-- 3) Extender tabla envio para operaciÃ³n de Ãºltima milla
ALTER TABLE envio ADD COLUMN IF NOT EXISTS id_motorizado INT NULL REFERENCES motorizado(id_motorizado);
ALTER TABLE envio ADD COLUMN IF NOT EXISTS asignado_por INT NULL REFERENCES usuario(id_usuario);
ALTER TABLE envio ADD COLUMN IF NOT EXISTS fecha_asignacion TIMESTAMPTZ NULL;
ALTER TABLE envio ADD COLUMN IF NOT EXISTS fecha_inicio_ruta TIMESTAMPTZ NULL;
ALTER TABLE envio ADD COLUMN IF NOT EXISTS fecha_entrega TIMESTAMPTZ NULL;
ALTER TABLE envio ADD COLUMN IF NOT EXISTS motivo_no_entrega TEXT NULL;

-- 4) Eventos operativos de delivery (bitÃ¡cora append-only)
CREATE TABLE IF NOT EXISTS delivery_event (
  id_delivery_event INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  id_envio INT NOT NULL REFERENCES envio(id_envio),
  id_pedido INT NOT NULL REFERENCES pedido(id_pedido),
  tipo_evento VARCHAR(50) NOT NULL,
  detalle TEXT NULL,
  payload_json TEXT NULL,
  id_usuario INT NULL REFERENCES usuario(id_usuario),
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

-- 5) Evidencia de entrega
CREATE TABLE IF NOT EXISTS entrega_evidencia (
  id_entrega_evidencia INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  id_envio INT NOT NULL REFERENCES envio(id_envio),
  id_pedido INT NOT NULL REFERENCES pedido(id_pedido),
  nombre_receptor VARCHAR(120) NOT NULL,
  dni_receptor VARCHAR(20) NULL,
  observacion TEXT NULL,
  foto_url VARCHAR(2048) NULL,
  lat DECIMAL(10,7) NULL,
  lng DECIMAL(10,7) NULL,
  id_usuario INT NULL REFERENCES usuario(id_usuario),
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

CREATE INDEX IF NOT EXISTS idx_envio_motorizado_estado ON envio (id_motorizado, estado_envio, fecha_asignacion);
CREATE INDEX IF NOT EXISTS idx_delivery_event_pedido ON delivery_event (id_pedido, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_entrega_evidencia_pedido ON entrega_evidencia (id_pedido, created_at DESC);

--- [SECCION] SEED CORE (04_seed_core.sql) ---

-- PostgreSQL seeds core

INSERT INTO metodos_de_pago (tipo_metodo, detalles) VALUES
('Yape', 'Pago mÃ³vil con Yape'),
('Plin', 'Pago mÃ³vil con Plin'),
('Transferencia', 'Transferencia bancaria'),
('Tarjeta', 'Visa / Mastercard');

INSERT INTO categoria (nombre_categoria) VALUES
('Abarrotes'),
('Bebidas'),
('LÃ¡cteos'),
('Frutas y Verduras'),
('Carnes'),
('Limpieza'),
('PanaderÃ­a'),
('Congelados'),
('Snacks'),
('Cuidado Personal');

INSERT INTO marca (nombre_marca) VALUES
('Gloria'),
('NestlÃ©'),
('Coca-Cola'),
('Alicorp'),
('San Fernando'),
('Laive'),
('Primor'),
('Molitalia'),
('Pepsico'),
('Unilever'),
('Oral-B'),
('Redondos'),
('El Frutero'),
('La Florencia'),
('Bimbo'),
('GenÃ©rico');

INSERT INTO proveedor (nombre_proveedor, telefono, email, direccion) VALUES
('Distribuidora Alfa', '987333331', 'alfa@dist.com', 'Av. Argentina 123'),
('Mayorista Beta', '987333332', 'beta@mayorista.com', 'Av. Brasil 456'),
('Importadora Gamma', '987333333', 'gamma@import.com', 'Av. Colombia 789'),
('Comercial Delta', '987333334', 'delta@comercial.com', 'Av. Ecuador 321'),
('Distribuidora Ã‰psilon', '987333335', 'epsilon@dist.com', 'Av. Venezuela 654'),
('Mayorista Zeta', '987333336', 'zeta@mayorista.com', 'Av. Bolivia 987'),
('Importadora Eta', '987333337', 'eta@import.com', 'Av. Paraguay 159'),
('Comercial Theta', '987333338', 'theta@comercial.com', 'Av. Uruguay 753'),
('Distribuidora Iota', '987333339', 'iota@dist.com', 'Av. Chile 951'),
('Mayorista Kappa', '987333340', 'kappa@mayorista.com', 'Av. PerÃº 357');

INSERT INTO almacen (nombre_almacen, direccion, telefono, responsable) VALUES
('AlmacÃ©n Central', 'Av. Industrial 123', '987222221', 'Luis RamÃ­rez'),
('AlmacÃ©n Norte', 'Av. TÃºpac Amaru 456', '987222222', 'Carlos GutiÃ©rrez'),
('AlmacÃ©n Sur', 'Av. Javier Prado 789', '987222223', 'Marta SÃ¡nchez'),
('AlmacÃ©n Este', 'Av. La Molina 321', '987222224', 'Jorge LÃ³pez');

INSERT INTO delivery (nombre_empresa, telefono, email) VALUES
('Delivery Express', '987123456', 'express@delivery.com'),
('RÃ¡pido EnvÃ­os', '987123457', 'rapido@envios.com'),
('Moto Mensajeros', '987123458', 'moto@mensajeros.com'),
('EnvÃ­os Seguros', '987123459', 'seguros@envios.com'),
('Veloz Delivery', '987123460', 'veloz@delivery.com'),
('Moto RÃ¡pidos', '987123461', 'moto@rapidos.com'),
('Express Motos', '987123462', 'express@motos.com'),
('Mensajeros YA', '987123463', 'ya@mensajeros.com'),
('EnvÃ­os Flash', '987123464', 'flash@envios.com'),
('RÃ¡pidos Moto', '987123465', 'rapidos@moto.com');

INSERT INTO motorizado (nombre, apellido, telefono, licencia, id_delivery) VALUES
('Roberto', 'GarcÃ­a', '987111111', 'LIC001', 1),
('Jorge', 'Vargas', '987111112', 'LIC002', 1),
('Daniel', 'Quispe', '987111113', 'LIC003', 2),
('Fernando', 'Salas', '987111114', 'LIC004', 2),
('Ricardo', 'Mendoza', '987111115', 'LIC005', 3),
('Alberto', 'Castro', '987111116', 'LIC006', 3),
('HÃ©ctor', 'Rojas', '987111117', 'LIC007', 4),
('Oscar', 'Paredes', '987111118', 'LIC008', 4),
('Pablo', 'Silva', '987111119', 'LIC009', 5),
('RaÃºl', 'DÃ­az', '987111120', 'LIC010', 5);
SELECT 1;

--- [SECCION] SEED DEMO (05_seed_demo.sql) ---

-- PostgreSQL seeds demo

-- USUARIOS (bcrypt hash para DEV)
-- Password DEV: Password1 (excepto cli@email.com, emp@email.com, admin@email.com => 123)
-- email_verificado=1 para no bloquear logins demo
INSERT INTO usuario (nombre, apellido, email, contrasena, telefono, direccion_principal, rol, email_verificado) VALUES
('Juan', 'PÃ©rez', 'cli@email.com', '$2a$10$tKJ56pQTAAvrb8Uerajsyuq5h30XvGp6NBaqwRGwZpF/ZkPsjUMPS', '987654321', 'Av. Lima 123', 'CLIENTE', true),
('MarÃ­a', 'GÃ³mez', 'emp@email.com', '$2a$10$tKJ56pQTAAvrb8Uerajsyuq5h30XvGp6NBaqwRGwZpF/ZkPsjUMPS', '987654322', 'Av. Arequipa 456', 'EMPLEADO', true),
('Carlos', 'LÃ³pez', 'admin@email.com', '$2a$10$tKJ56pQTAAvrb8Uerajsyuq5h30XvGp6NBaqwRGwZpF/ZkPsjUMPS', '987654323', 'Jr. Cusco 789', 'ADMINISTRADOR', true),
('Diego', 'Reyes', 'repartidor@email.com', '$2a$10$tKJ56pQTAAvrb8Uerajsyuq5h30XvGp6NBaqwRGwZpF/ZkPsjUMPS', '987654331', 'Av. Delivery 100', 'REPARTIDOR', true),
('Ana', 'RodrÃ­guez', 'ana@email.com', '$2a$10$B5o9vCtVsQC9xGNbeQjfleBj.ZpUNIK/3uO8KJwCUVsOTiedHdu0K', '987654324', 'Av. Tacna 321', 'CLIENTE', true),
('Luis', 'MartÃ­nez', 'luis@email.com', '$2a$10$B5o9vCtVsQC9xGNbeQjfleBj.ZpUNIK/3uO8KJwCUVsOTiedHdu0K', '987654325', 'Jr. Ayacucho 654', 'CLIENTE', true),
('Pedro', 'SÃ¡nchez', 'pedro@email.com', '$2a$10$B5o9vCtVsQC9xGNbeQjfleBj.ZpUNIK/3uO8KJwCUVsOTiedHdu0K', '987654326', 'Av. BolÃ­var 987', 'ADMINISTRADOR', true),
('Laura', 'DÃ­az', 'laura@email.com', '$2a$10$B5o9vCtVsQC9xGNbeQjfleBj.ZpUNIK/3uO8KJwCUVsOTiedHdu0K', '987654327', 'Jr. Huancavelica 654', 'EMPLEADO', true),
('SofÃ­a', 'HernÃ¡ndez', 'sofia@email.com', '$2a$10$B5o9vCtVsQC9xGNbeQjfleBj.ZpUNIK/3uO8KJwCUVsOTiedHdu0K', '987654328', 'Av. Salaverry 321', 'CLIENTE', true),
('Miguel', 'Torres', 'miguel@email.com', '$2a$10$B5o9vCtVsQC9xGNbeQjfleBj.ZpUNIK/3uO8KJwCUVsOTiedHdu0K', '987654329', 'Jr. JunÃ­n 159', 'CLIENTE', true),
('Elena', 'Ruiz', 'elena@email.com', '$2a$10$B5o9vCtVsQC9xGNbeQjfleBj.ZpUNIK/3uO8KJwCUVsOTiedHdu0K', '987654330', 'Av. Brasil 753', 'CLIENTE', true);

-- Vincular usuario repartidor demo con un motorizado
UPDATE motorizado
SET id_usuario = (SELECT id_usuario FROM usuario WHERE email = 'repartidor@email.com')
WHERE id_motorizado = 1;

-- PRODUCTOS
INSERT INTO producto (nombre_producto, descripcion, precio, imagen, id_categoria, id_marca) VALUES
('Leche Entera Gloria 1L', 'Leche entera en caja 1 L', 4.50, '/api/uploads/images/leche-gloria-1l.webp', 3, 1),
('Arroz CosteÃ±o 5 kg', 'Arroz extra 5 kg', 18.90, '/api/uploads/images/arroz-costeno-5kg.webp', 1, 4),
('Coca-Cola 3 L', 'Gaseosa sabor original 3 L', 10.50, '/api/uploads/images/coca-cola-3l.webp', 2, 3),
('Aceite Primor 900 ml', 'Aceite vegetal', 8.20, '/api/uploads/images/aceite-primor-900.webp', 1, 7),
('Pollo Entero San Fernando 1 kg', 'Pollo entero 1 kg', 12.80, '/api/uploads/images/pollo-sf-1kg.webp', 5, 5),
('Yogur Gloria Fresa 1 L', 'Yogur sabor fresa 1 litro', 6.30, '/api/uploads/images/yogur-fresa-1l.webp', 3, 1),
('Spaghetti Don Vittorio 500 g', 'Pasta spaghetti 500 g', 3.20, '/api/uploads/images/spaghetti-500g.webp', 1, 4),
('Jugo Del Valle 1 L', 'Jugo de frutas surtidas 1 L', 5.90, '/api/uploads/images/jugo-del-valle-1l.webp', 2, 3),
('JabÃ³n BolÃ­var 200 g', 'JabÃ³n de lavar ropa 200 g', 4.80, '/api/uploads/images/jabon-bolivar-200g.webp', 6, 4),
('Papas Fritas Lay''s 140 g', 'Snack de papas 140 g', 6.50, '/api/uploads/images/lays-140g.webp', 9, 10),
('Carne Molida El Buen Corte', 'Carne molida de res', 18.50, '/api/uploads/images/carne-molida.webp', 8, 5),
('Cepillo ElÃ©ctrico Oral-B Pro Series 3', 'Cepillo dental elÃ©ctrico Oral-B Pro Series 3', 125.00, '/api/uploads/images/cepillo-electrico-oralb.webp', 10, 11),
('Doritos Queso Atrevido 100 g', 'Tortillas fritas de maÃ­z sabor a queso 100 g', 7.50, '/api/uploads/images/doritos-queso-atrevido-100g.webp', 9, 9),
('Fresa Entera Congelada El Frutero 1 kg', 'Fresa entera congelada 1 kg', 18.90, '/api/uploads/images/fresa-entera-congelada-el-frutero-1kg.webp', 8, 13),
('Hamburguesa de Pollo Redondos 360 g', 'Hamburguesa de pollo empanizada 360 g', 15.90, '/api/uploads/images/hamburguesa-pollo-redondos-360g.webp', 8, 12),
('Manzana Roja 1 kg', 'Manzana roja fresca 1 kg', 7.90, '/api/uploads/images/manzana-roja-1kg.webp', 4, 16),
('Pan Carioca La Florencia 400 g', 'Pan carioca en bolsa 400 g', 6.50, '/api/uploads/images/pan-carioca-la-florencia-400g.webp', 7, 14),
('Pan de Molde Blanco La Florencia 500 g', 'Pan de molde blanco 500 g', 7.50, '/api/uploads/images/pan-molde-blanco-la-florencia-500g.webp', 7, 14),
('Pan de Molde Blanco XL Bimbo 770 g', 'Pan de molde blanco XL 770 g', 12.90, '/api/uploads/images/pan-molde-blanco-bimbo-xl-770g.webp', 7, 15),
('Piqueo Snax Original 110 g', 'Snack piqueo sabor original 110 g', 6.90, '/api/uploads/images/piqueo-snax-original-110g.webp', 9, 9);


-- PROVEEDOR_PRODUCTO
INSERT INTO proveedor_producto (precio_unitario, precio_costo, fecha_ultima_compra, id_producto, id_proveedor) VALUES
(3.50, 4.50, '2025-10-01', 1, 1),
(15.00, 17.00, '2025-10-02', 2, 2),
(8.00, 9.00, '2025-10-03', 3, 3),
(6.00, 7.00, '2025-10-04', 4, 4),
(10.00, 11.00, '2025-10-05', 5, 5),
(4.80, 5.50, '2025-10-06', 6, 6),
(2.40, 2.80, '2025-10-07', 7, 7),
(4.30, 5.00, '2025-10-08', 8, 8),
(3.80, 4.20, '2025-10-09', 9, 9),
(5.00, 5.80, '2025-10-10', 10, 10),
(16.50, 14.00, '2025-10-11', 11, 5),
(105.00, 95.00, '2025-10-12', 12, 3),
(6.00, 5.20, '2025-10-13', 13, 2),
(15.50, 13.50, '2025-10-14', 14, 1),
(13.00, 11.20, '2025-10-15', 15, 4),
(6.20, 5.00, '2025-10-16', 16, 6),
(7.00, 5.80, '2025-10-17', 17, 6),
(11.50, 9.80, '2025-10-18', 18, 7),
(5.80, 4.90, '2025-10-19', 19, 2),
(5.50, 4.70, '2025-10-20', 20, 2);

-- INVENTARIO
INSERT INTO inventario (cantidad_disponible, id_producto, id_almacen) VALUES
(100, 1, 1),
(150, 2, 1),
(200, 3, 1),
(80, 4, 1),
(50, 5, 1),
(120, 6, 2),
(90, 7, 2),
(180, 8, 2),
(70, 9, 3),
(110, 10, 3),
(45, 11, 1),
(25, 12, 1),
(60, 13, 2),
(40, 14, 2),
(55, 15, 1),
(70, 16, 3),
(60, 17, 3),
(45, 18, 1),
(80, 19, 3),
(75, 20, 2);

-- ENTRADA_INVENTARIO
INSERT INTO entrada_inventario (cantidad_recibida, fecha_entrada, motivo_entrada, id_inventario) VALUES
(20, '2025-01-05', 'Compra', 1),
(35, '2025-01-18', 'Compra', 6),
(25, '2025-02-07', 'Compra', 2),
(40, '2025-02-20', 'Compra', 8),
(30, '2025-03-03', 'Compra', 3),
(15, '2025-03-19', 'Compra', 9),
(22, '2025-04-09', 'Compra', 4),
(28, '2025-04-26', 'Compra', 7),
(18, '2025-05-06', 'Compra', 5),
(33, '2025-05-24', 'Compra', 10),
(26, '2025-06-04', 'Compra', 6),
(31, '2025-06-20', 'Compra', 1),
(24, '2025-07-08', 'Compra', 7),
(29, '2025-07-23', 'Compra', 2),
(27, '2025-08-11', 'Compra', 8),
(20, '2025-08-28', 'Compra', 3),
(23, '2025-09-02', 'Compra', 9),
(34, '2025-09-16', 'Compra', 4),
(50, '2025-10-01', 'Compra', 1),
(30, '2025-10-05', 'Compra', 2),
(40, '2025-10-06', 'Compra', 3),
(20, '2025-10-07', 'DevoluciÃ³n', 4),
(15, '2025-10-11', 'Compra', 5),
(25, '2025-10-12', 'Compra', 6),
(35, '2025-10-13', 'Compra', 7),
(45, '2025-10-21', 'DevoluciÃ³n', 8),
(10, '2025-10-21', 'Compra', 9),
(55, '2025-10-22', 'Compra', 10),
(28, '2025-11-02', 'Compra', 5),
(19, '2025-11-15', 'Compra', 10),
(30, '2025-12-07', 'Compra', 6),
(25, '2025-12-18', 'Compra', 1);

-- SALIDA_INVENTARIO
INSERT INTO salida_inventario (cantidad_salida, fecha_salida, motivo_salida, id_inventario, id_usuario) VALUES
(9, '2025-01-06', 'Venta', 1, 2),
(12, '2025-01-21', 'Venta', 6, 7),
(11, '2025-02-08', 'Venta', 2, 2),
(8, '2025-02-22', 'Venta', 8, 7),
(10, '2025-03-04', 'Venta', 3, 7),
(7, '2025-03-20', 'Venta', 9, 2),
(13, '2025-04-10', 'Venta', 4, 7),
(9, '2025-04-27', 'Venta', 7, 2),
(8, '2025-05-07', 'Venta', 5, 2),
(14, '2025-05-25', 'Venta', 10, 7),
(10, '2025-06-05', 'Venta', 6, 7),
(12, '2025-06-21', 'Venta', 1, 2),
(9, '2025-07-09', 'Venta', 7, 2),
(11, '2025-07-24', 'Venta', 2, 7),
(10, '2025-08-12', 'Venta', 8, 7),
(8, '2025-08-29', 'Venta', 3, 2),
(9, '2025-09-03', 'Venta', 9, 2),
(13, '2025-09-17', 'Venta', 4, 7),
(13, '2025-10-01', 'Venta', 1, 2),
(15, '2025-10-02', 'Venta', 2, 2),
(14, '2025-10-03', 'Venta', 3, 7),
(28, '2025-10-04', 'Venta', 4, 7),
(26, '2025-10-05', 'Venta', 5, 2),
(19, '2025-10-06', 'Venta', 6, 7),
(18, '2025-10-07', 'Venta', 7, 2),
(11, '2025-10-08', 'Venta', 8, 7),
(14, '2025-10-09', 'Venta', 9, 7),
(21, '2025-10-10', 'Venta', 10, 2),
(12, '2025-11-03', 'Venta', 5, 2),
(10, '2025-11-16', 'Venta', 10, 7),
(11, '2025-12-08', 'Venta', 6, 7),
(9, '2025-12-19', 'Venta', 1, 2);

-- PEDIDO
INSERT INTO pedido (id_pedido, fecha_creacion, estado_pedido, total_pedido, costo_envio, direccion_envio, id_usuario, id_metodo_pago) VALUES
(1,  '2025-01-05', 'ENTREGADO', 19.50, 5.00, 'Av. Lima 123', 1, 1),
(2,  '2025-01-20', 'EN CAMINO', 25.30, 5.00, 'Av. Arequipa 456', 4, 2),
(3,  '2025-02-07', 'ENTREGADO', 25.80, 5.00, 'Jr. Cusco 789', 5, 3),
(4,  '2025-02-21', 'PENDIENTE', 28.20, 5.00, 'Av. Tacna 321', 8, 4),
(5,  '2025-03-03', 'ENTREGADO', 22.40, 5.00, 'Jr. Ayacucho 654', 9, 1),
(6,  '2025-03-18', 'CANCELADO', 31.70, 5.00, 'Av. BolÃ­var 987', 10, 2),
(7,  '2025-04-09', 'ENTREGADO', 33.60, 5.00, 'Av. Lima 123', 1, 3),
(8,  '2025-04-25', 'EN CAMINO', 20.50, 5.00, 'Av. Arequipa 456', 4, 4),
(9,  '2025-05-06', 'ENTREGADO', 18.30, 5.00, 'Jr. Cusco 789', 5, 1),
(10, '2025-05-22', 'PENDIENTE', 46.00, 5.00, 'Av. Tacna 321', 8, 2),
(11, '2025-06-04', 'ENTREGADO', 22.20, 5.00, 'Jr. Ayacucho 654', 9, 3),
(12, '2025-06-19', 'PREPARADO', 44.30, 5.00, 'Av. BolÃ­var 987', 10, 4),
(13, '2025-07-08', 'ENTREGADO', 22.00, 5.00, 'Av. Lima 123', 1, 1),
(14, '2025-07-23', 'EN CAMINO', 30.50, 5.00, 'Av. Arequipa 456', 4, 2),
(15, '2025-08-11', 'ENTREGADO', 44.10, 5.00, 'Jr. Cusco 789', 5, 3),
(16, '2025-08-27', 'PENDIENTE', 33.80, 5.00, 'Av. Tacna 321', 8, 4),
(17, '2025-09-02', 'ENTREGADO', 21.20, 5.00, 'Jr. Ayacucho 654', 9, 1),
(18, '2025-09-16', 'PREPARADO', 25.30, 5.00, 'Av. BolÃ­var 987', 10, 2),
(19, '2025-10-01', 'ENTREGADO', 45.50, 5.00, 'Av. Lima 123', 1, 1),
(20, '2025-10-02', 'EN CAMINO', 38.70, 5.00, 'Av. Arequipa 456', 4, 2),
(21, '2025-10-03', 'PENDIENTE', 52.30, 5.00, 'Jr. Cusco 789', 5, 3),
(22, '2025-10-04', 'ENTREGADO', 27.90, 5.00, 'Av. Tacna 321', 8, 4),
(23, '2025-10-05', 'EN CAMINO', 64.20, 5.00, 'Jr. Ayacucho 654', 9, 1),
(24, '2025-10-06', 'ENTREGADO', 19.80, 5.00, 'Av. BolÃ­var 987', 10, 2),
(25, '2025-10-07', 'PENDIENTE', 42.50, 5.00, 'Jr. Huancavelica 654', 1, 3),
(26, '2025-10-08', 'ENTREGADO', 35.70, 5.00, 'Av. Salaverry 321', 4, 4),
(27, '2025-10-09', 'EN CAMINO', 58.90, 5.00, 'Jr. JunÃ­n 159', 5, 1),
(28, '2025-10-10', 'PENDIENTE', 23.40, 5.00, 'Av. Brasil 753', 8, 2),
(29, '2025-11-01', 'ENTREGADO', 45.50, 5.00, 'Av. Lima 123', 9, 1),
(30, '2025-11-02', 'EN CAMINO', 38.70, 5.00, 'Av. Arequipa 456', 10, 2),
(31, '2025-11-03', 'ENTREGADO', 52.30, 5.00, 'Jr. Cusco 789', 1, 3),
(32, '2025-11-04', 'ENTREGADO', 27.90, 5.00, 'Av. Tacna 321', 4, 4),
(33, '2025-11-05', 'EN CAMINO', 64.20, 5.00, 'Jr. Ayacucho 654', 5, 1),
(34, '2025-12-06', 'ENTREGADO', 19.80, 5.00, 'Av. BolÃ­var 987', 8, 2),
(35, '2025-12-07', 'PENDIENTE', 42.50, 5.00, 'Jr. Huancavelica 654', 9, 3),
(36, '2025-12-08', 'ENTREGADO', 35.70, 5.00, 'Av. Salaverry 321', 10, 4),
(37, '2025-12-09', 'ENTREGADO', 58.90, 5.00, 'Jr. JunÃ­n 159', 1, 1),
(38, '2025-12-10', 'ENTREGADO', 23.40, 5.00, 'Av. Brasil 753', 4, 2);

SELECT setval(pg_get_serial_sequence('pedido','id_pedido'), (SELECT MAX(id_pedido) FROM pedido));

-- ENVIO
INSERT INTO envio (numero_rastreo, transportista, fecha_envio, estado_envio, costo_envio, id_pedido, id_delivery) VALUES
('TRK021', 'Delivery Express', '2025-01-06', 'ENTREGADO', 5.00, 1, 1),
('TRK022', 'RÃ¡pido EnvÃ­os', '2025-01-21', 'EN CAMINO', 5.00, 2, 2),
('TRK023', 'Moto Mensajeros', '2025-02-08', 'ENTREGADO', 5.00, 3, 3),
('TRK025', 'Veloz Delivery', '2025-03-04', 'ENTREGADO', 5.00, 5, 5),
('TRK027', 'Express Motos', '2025-04-10', 'ENTREGADO', 5.00, 7, 2),
('TRK028', 'Mensajeros YA', '2025-04-26', 'EN CAMINO', 5.00, 8, 3),
('TRK029', 'EnvÃ­os Flash', '2025-05-07', 'ENTREGADO', 5.00, 9, 4),
('TRK031', 'Delivery Express', '2025-06-05', 'ENTREGADO', 5.00, 11, 1),
('TRK033', 'Moto Mensajeros', '2025-07-09', 'ENTREGADO', 5.00, 13, 3),
('TRK034', 'EnvÃ­os Seguros', '2025-07-24', 'EN CAMINO', 5.00, 14, 4),
('TRK035', 'Veloz Delivery', '2025-08-12', 'ENTREGADO', 5.00, 15, 5),
('TRK037', 'Express Motos', '2025-09-03', 'ENTREGADO', 5.00, 17, 2),
('TRK001', 'Delivery Express', '2025-10-01', 'ENTREGADO', 5.00, 19, 1),
('TRK002', 'RÃ¡pido EnvÃ­os', '2025-10-02', 'EN CAMINO', 5.00, 20, 2),
('TRK004', 'EnvÃ­os Seguros', '2025-10-04', 'ENTREGADO', 5.00, 22, 4),
('TRK005', 'Veloz Delivery', '2025-10-05', 'EN CAMINO', 5.00, 23, 5),
('TRK006', 'Moto RÃ¡pidos', '2025-10-06', 'ENTREGADO', 5.00, 24, 1),
('TRK008', 'Mensajeros YA', '2025-10-08', 'ENTREGADO', 5.00, 26, 3),
('TRK009', 'EnvÃ­os Flash', '2025-10-09', 'EN CAMINO', 5.00, 27, 4),
('TRK011', 'Delivery Express', '2025-11-01', 'ENTREGADO', 5.00, 29, 1),
('TRK012', 'RÃ¡pido EnvÃ­os', '2025-11-02', 'EN CAMINO', 5.00, 30, 2),
('TRK013', 'Moto Mensajeros', '2025-11-03', 'ENTREGADO', 5.00, 31, 3),
('TRK014', 'EnvÃ­os Seguros', '2025-11-04', 'ENTREGADO', 5.00, 32, 4),
('TRK015', 'Veloz Delivery', '2025-11-05', 'EN CAMINO', 5.00, 33, 5),
('TRK016', 'Moto RÃ¡pidos', '2025-12-06', 'ENTREGADO', 5.00, 34, 1),
('TRK018', 'Mensajeros YA', '2025-12-08', 'ENTREGADO', 5.00, 36, 3),
('TRK019', 'EnvÃ­os Flash', '2025-12-09', 'ENTREGADO', 5.00, 37, 4),
('TRK020', 'RÃ¡pidos Moto', '2025-12-10', 'ENTREGADO', 5.00, 38, 5),
('TRK003', 'Moto Mensajeros', NULL, 'PENDIENTE', 5.00, 21, 3),
('TRK007', 'Express Motos', NULL, 'PENDIENTE', 5.00, 25, 2),
('TRK010', 'RÃ¡pidos Moto', NULL, 'PENDIENTE', 5.00, 28, 5),
('TRK017', 'Express Motos', NULL, 'PENDIENTE', 5.00, 35, 2),
('TRK024', 'EnvÃ­os Seguros', NULL, 'PENDIENTE', 5.00, 4, 4),
('TRK026', 'Moto RÃ¡pidos', NULL, 'PENDIENTE', 5.00, 6, 1),
('TRK030', 'RÃ¡pidos Moto', NULL, 'PENDIENTE', 5.00, 10, 5),
('TRK032', 'RÃ¡pido EnvÃ­os', NULL, 'PENDIENTE', 5.00, 12, 2),
('TRK036', 'Moto RÃ¡pidos', NULL, 'PENDIENTE', 5.00, 16, 1),
('TRK038', 'Mensajeros YA', NULL, 'PENDIENTE', 5.00, 18, 3);

-- COMPROBANTE
INSERT INTO comprobante (tipo_comprobante, numero_comprobante, fecha_creacion, monto_total, estado_comprobante, fecha_emision, ultima_actualizacion, id_pedido, id_metodo_pago) VALUES
('BOLETA',  'B001-0011', '2025-01-05', 24.50, 'PAGADO',    '2025-01-05', '2025-01-05', 1, 1),
('FACTURA', 'F001-0011', '2025-01-20', 30.30, 'PAGADO',    '2025-01-20', '2025-01-20', 2, 2),
('BOLETA',  'B001-0012', '2025-02-07', 30.80, 'PAGADO',    '2025-02-07', '2025-02-07', 3, 3),
('FACTURA', 'F001-0012', '2025-02-21', 33.20, 'PENDIENTE', NULL,        '2025-02-21', 4, 4),
('BOLETA',  'B001-0013', '2025-03-03', 27.40, 'PAGADO',    '2025-03-03', '2025-03-03', 5, 1),
('FACTURA', 'F001-0013', '2025-03-18', 36.70, 'ANULADO',   '2025-03-19', '2025-03-19', 6, 2),
('BOLETA',  'B001-0014', '2025-04-09', 38.60, 'PAGADO',    '2025-04-09', '2025-04-09', 7, 3),
('FACTURA', 'F001-0014', '2025-04-25', 25.50, 'PAGADO',    '2025-04-25', '2025-04-25', 8, 4),
('BOLETA',  'B001-0015', '2025-05-06', 23.30, 'PAGADO',    '2025-05-06', '2025-05-06', 9, 1),
('FACTURA', 'F001-0015', '2025-05-22', 51.00, 'PENDIENTE', NULL,        '2025-05-22', 10, 2),
('BOLETA',  'B001-0016', '2025-06-04', 27.20, 'PAGADO',    '2025-06-04', '2025-06-04', 11, 3),
('FACTURA', 'F001-0016', '2025-06-19', 49.30, 'PENDIENTE', NULL,        '2025-06-19', 12, 4),
('BOLETA',  'B001-0017', '2025-07-08', 27.00, 'PAGADO',    '2025-07-08', '2025-07-08', 13, 1),
('FACTURA', 'F001-0017', '2025-07-23', 35.50, 'PAGADO',    '2025-07-23', '2025-07-23', 14, 2),
('BOLETA',  'B001-0018', '2025-08-11', 49.10, 'PAGADO',    '2025-08-11', '2025-08-11', 15, 3),
('FACTURA', 'F001-0018', '2025-08-27', 38.80, 'PENDIENTE', NULL,        '2025-08-27', 16, 4),
('BOLETA',  'B001-0019', '2025-09-02', 26.20, 'PAGADO',    '2025-09-02', '2025-09-02', 17, 1),
('FACTURA', 'F001-0019', '2025-09-16', 30.30, 'PENDIENTE', NULL,        '2025-09-16', 18, 2),
('BOLETA',  'B001-0001', '2025-10-01', 50.50, 'PAGADO',    '2025-10-01', '2025-10-01', 19, 1),
('FACTURA', 'F001-0001', '2025-10-02', 43.70, 'PAGADO',    '2025-10-02', '2025-10-02', 20, 2),
('BOLETA',  'B001-0002', '2025-10-03', 57.30, 'ANULADO',   '2025-10-05', '2025-10-05', 21, 3),
('FACTURA', 'F001-0002', '2025-10-04', 32.90, 'PAGADO',    '2025-10-04', '2025-10-04', 22, 4),
('BOLETA',  'B001-0003', '2025-10-05', 69.20, 'PAGADO',    '2025-10-05', '2025-10-05', 23, 1),
('FACTURA', 'F001-0003', '2025-10-06', 24.80, 'PENDIENTE',  NULL,        '2025-10-06', 24, 2),
('BOLETA',  'B001-0004', '2025-10-07', 47.50, 'PENDIENTE',  NULL,        '2025-10-07', 25, 3),
('FACTURA', 'F001-0004', '2025-10-08', 40.70, 'PENDIENTE',  NULL,        '2025-10-08', 26, 4),
('BOLETA',  'B001-0005', '2025-10-09', 63.90, 'OBSERVADO', '2025-10-09', '2025-10-09', 27, 1),
('FACTURA', 'F001-0005', '2025-10-10', 28.40, 'PAGADO',    '2025-10-10', '2025-10-10', 28, 2),
('BOLETA',  'B001-0006', '2025-11-01', 50.50, 'PAGADO',    '2025-11-01', '2025-11-01', 29, 1),
('FACTURA', 'F001-0006', '2025-11-02', 43.70, 'PAGADO',    '2025-11-02', '2025-11-02', 30, 2),
('BOLETA',  'B001-0007', '2025-11-03', 57.30, 'OBSERVADO', '2025-11-03', '2025-11-03', 31, 3),
('FACTURA', 'F001-0007', '2025-11-04', 32.90, 'PAGADO',    '2025-11-04', '2025-11-04', 32, 4),
('BOLETA',  'B001-0008', '2025-11-05', 69.20, 'PAGADO',    '2025-11-05', '2025-11-05', 33, 1),
('FACTURA', 'F001-0008', '2025-12-06', 24.80, 'PENDIENTE', NULL,         '2025-12-06', 34, 2),
('BOLETA',  'B001-0009', '2025-12-07', 47.50, 'ANULADO',   '2025-12-07', '2025-12-07', 35, 3),
('FACTURA', 'F001-0009', '2025-12-08', 40.70, 'PENDIENTE', NULL,         '2025-12-08', 36, 4),
('BOLETA',  'B001-0010', '2025-12-09', 63.90, 'OBSERVADO', '2025-12-09', '2025-12-09', 37, 1),
('FACTURA', 'F001-0010', '2025-12-10', 28.40, 'PAGADO',    '2025-12-10', '2025-12-10', 38, 2);

-- BOLETA
INSERT INTO boleta (nombre_cliente, dni_cliente, id_comprobante) VALUES
('Juan PÃ©rez', '12345678', 1),
('Carlos LÃ³pez', '23456789', 3),
('Luis MartÃ­nez', '34567890', 5),
('Laura DÃ­az', '45678901', 7),
('Elena Ruiz', '56789012', 9);

-- FACTURA
INSERT INTO factura (monto_base, monto_impuesto, razon_social_cliente, ruc_cliente, direccion_cliente, id_comprobante) VALUES
(41.30, 2.40, 'MarÃ­a GÃ³mez EIRL', '20123456789', 'Av. Arequipa 456', 2),
(30.60, 2.30, 'Ana RodrÃ­guez SAC', '20234567890', 'Av. Tacna 321', 4),
(22.20, 2.60, 'Pedro SÃ¡nchez EIRL', '20345678901', 'Av. BolÃ­var 987', 6),
(34.50, 6.20, 'SofÃ­a HernÃ¡ndez SAC', '20456789012', 'Av. Salaverry 321', 8),
(24.00, 4.40, 'Miguel Torres EIRL', '20567890123', 'Jr. JunÃ­n 159', 10);

-- DETALLE_PEDIDO
INSERT INTO detalle_pedido (cantidad, precio_unitario_venta, subtotal, id_pedido, id_producto) VALUES
(2, 4.50, 9.00, 19, 1),
(1, 18.90, 18.90, 19, 2),
(1, 10.50, 10.50, 20, 3),
(2, 8.20, 16.40, 20, 4),
(1, 12.80, 12.80, 21, 5),
(2, 6.30, 12.60, 21, 6),
(3, 3.20, 9.60, 22, 7),
(2, 5.90, 11.80, 22, 8),
(1, 4.80, 4.80, 23, 9),
(2, 6.50, 13.00, 23, 10),
(2, 4.50, 9.00, 24, 1),
(1, 18.90, 18.90, 24, 2),
(1, 10.50, 10.50, 25, 3),
(2, 8.20, 16.40, 25, 4),
(1, 12.80, 12.80, 26, 5),
(2, 6.30, 12.60, 26, 6),
(3, 3.20, 9.60, 27, 7),
(2, 5.90, 11.80, 27, 8),
(1, 4.80, 4.80, 28, 9),
(2, 6.50, 13.00, 28, 10),
(2, 4.50, 9.00, 29, 1),
(1, 18.90, 18.90, 29, 2),
(1, 10.50, 10.50, 30, 3),
(2, 8.20, 16.40, 30, 4),
(1, 12.80, 12.80, 31, 5),
(2, 6.30, 12.60, 31, 6),
(3, 3.20, 9.60, 32, 7),
(2, 5.90, 11.80, 32, 8),
(1, 4.80, 4.80, 33, 9),
(2, 6.50, 13.00, 33, 10),
(2, 4.50, 9.00, 34, 1),
(1, 18.90, 18.90, 34, 2),
(1, 10.50, 10.50, 35, 3),
(2, 8.20, 16.40, 35, 4),
(1, 12.80, 12.80, 36, 5),
(2, 6.30, 12.60, 36, 6),
(3, 3.20, 9.60, 37, 7),
(2, 5.90, 11.80, 37, 8),
(1, 4.80, 4.80, 38, 9),
(2, 6.50, 13.00, 38, 10),
(2, 4.50, 9.00, 1, 1),
(1, 10.50, 10.50, 1, 3),
(1, 18.90, 18.90, 2, 2),
(2, 3.20, 6.40, 2, 7),
(3, 6.50, 19.50, 3, 10),
(1, 6.30, 6.30, 3, 6),
(2, 8.20, 16.40, 4, 4),
(2, 5.90, 11.80, 4, 8),
(1, 12.80, 12.80, 5, 5),
(2, 4.80, 9.60, 5, 9),
(4, 3.20, 12.80, 6, 7),
(1, 18.90, 18.90, 6, 2),
(2, 6.30, 12.60, 7, 6),
(2, 10.50, 21.00, 7, 3),
(1, 4.50, 4.50, 8, 1),
(5, 3.20, 16.00, 8, 7),
(2, 5.90, 11.80, 9, 8),
(1, 6.50, 6.50, 9, 10),
(1, 8.20, 8.20, 10, 4),
(2, 18.90, 37.80, 10, 2),
(2, 4.80, 9.60, 11, 9),
(2, 6.30, 12.60, 11, 6),
(3, 10.50, 31.50, 12, 3),
(1, 12.80, 12.80, 12, 5),
(2, 4.50, 9.00, 13, 1),
(2, 6.50, 13.00, 13, 10),
(1, 5.90, 5.90, 14, 8),
(3, 8.20, 24.60, 14, 4),
(2, 18.90, 37.80, 15, 2),
(1, 6.30, 6.30, 15, 6),
(1, 12.80, 12.80, 16, 5),
(2, 10.50, 21.00, 16, 3),
(2, 8.20, 16.40, 17, 4),
(1, 4.80, 4.80, 17, 9),
(3, 6.30, 18.90, 18, 6),
(2, 3.20, 6.40, 18, 7);

-- DEVOLUCION
INSERT INTO devolucion (fecha_solicitud, estado_devolucion, razon_devolucion, id_pedido, id_usuario) VALUES
('2025-03-05', 'APROBADA', 'Producto daÃ±ado en la entrega', 5, 9),
('2025-06-06', 'PENDIENTE', 'Producto equivocado', 11, 9),
('2025-09-05', 'APROBADA', 'Empaque daÃ±ado', 17, 9),
('2025-10-02', 'APROBADA', 'Leche vencida', 19, 1),
('2025-10-03', 'PENDIENTE', 'Aceite derramado', 22, 4),
('2025-10-05', 'APROBADA', 'Pollo en mal estado', 23, 5),
('2025-10-07', 'RECHAZADA', 'Producto no gustÃ³', 25, 7),
('2025-10-09', 'APROBADA', 'JabÃ³n sin empaque', 27, 9);

-- DETALLE_DEVOLUCION
INSERT INTO detalle_devolucion (cantidad_devuelta, condicion_producto, precio_unitario_devolucion, id_devolucion, id_detalle_pedido) VALUES
(1, 'DAÃ‘ADO', 4.80, 1, 50),
(1, 'EQUIVOCADO', 6.30, 2, 62),
(1, 'EMPAQUE DAÃ‘ADO', 4.80, 3, 74),
(1, 'VENCIDO', 4.50, 4, 1),
(1, 'DERRAMADO', 8.20, 5, 4),
(1, 'MAL ESTADO', 12.80, 6, 5),
(1, 'BUEN ESTADO', 3.20, 7, 7),
(1, 'SIN EMPAQUE', 4.80, 8, 9);

-- RECLAMO
INSERT INTO reclamo (tipo_reclamo, descripcion_problema, estado_reclamo, fecha_creacion, id_usuario) VALUES
('ENTREGA TARDÃA','Entrega con retraso','RESUELTO','2025-01-22',4),
('PRODUCTO DAÃ‘ADO','Producto llegÃ³ roto','RESUELTO','2025-03-06',9),
('ERROR EN FACTURA','Datos incorrectos en el comprobante','EN REVISIÃ“N','2025-05-23',8),
('FALTA PRODUCTO','FaltÃ³ un Ã­tem del pedido','PENDIENTE','2025-07-10',1),
('PRODUCTO VENCIDO','Producto cercano a vencimiento','EN REVISIÃ“N','2025-09-06',9),
('PRODUCTO DAÃ‘ADO','Leche llegÃ³ vencida','RESUELTO','2025-10-01',1),
('ENTREGA TARDÃA','Pedido llegÃ³ con retraso','EN REVISIÃ“N','2025-10-02',2),
('FALTA PRODUCTO','FaltÃ³ el yogurt','RESUELTO','2025-10-03',3),
('PRODUCTO EQUIVOCADO','Enviaron fideos incorrectos','PENDIENTE','2025-10-04',4),
('EMPAQUE DAÃ‘ADO','Aceite llegÃ³ derramado','RESUELTO','2025-10-05',5),
('ENTREGA INCOMPLETA','Falta una manzana','EN REVISIÃ“N','2025-10-06',6),
('PRODUCTO VENCIDO','Papas fritas vencidas','PENDIENTE','2025-10-07',7),
('MAL ESTADO','Pollo con mal olor','RESUELTO','2025-10-08',8),
('ERROR EN FACTURA','Monto incorrecto','EN REVISIÃ“N','2025-10-09',9),
('PRODUCTO ABIERTO','JabÃ³n sin empaque','PENDIENTE','2025-10-10',10),
('ENTREGA INCOMPLETA','Paquete incompleto','PENDIENTE','2025-12-12',10);

-- HISTORIAL
INSERT INTO historial (descripcion, accion, fecha_accion, id_reclamo, id_pedido, id_usuario) VALUES
('Seguimiento de retraso','VERIFICACIÃ“N','2025-01-23',1,2,6),
('ReposiciÃ³n por producto daÃ±ado','REEMPLAZO PRODUCTO','2025-03-07',2,5,6),
('CorrecciÃ³n de datos de comprobante','CORRECCIÃ“N FACTURA','2025-05-24',3,10,6),
('EnvÃ­o de Ã­tem faltante','ENVÃO PRODUCTO','2025-07-11',4,13,6),
('RevisiÃ³n por producto vencido','VERIFICACIÃ“N','2025-09-07',5,17,6),
('Se reemplazÃ³ producto daÃ±ado','REEMPLAZO PRODUCTO','2025-10-02',6,19,6),
('VerificaciÃ³n de retraso en entrega','VERIFICACIÃ“N','2025-10-03',7,20,6),
('EnvÃ­o de producto faltante','ENVÃO PRODUCTO','2025-10-04',8,21,6),
('ValidaciÃ³n de producto equivocado','VERIFICACIÃ“N','2025-10-05',9,22,6),
('Reembolso por empaque daÃ±ado','REEMBOLSO','2025-10-06',10,23,6),
('RevisiÃ³n de entrega incompleta','VERIFICACIÃ“N','2025-10-07',11,24,6),
('InspecciÃ³n por producto vencido','VERIFICACIÃ“N','2025-10-08',12,25,6),
('Reemplazo de producto en mal estado','REEMPLAZO PRODUCTO','2025-10-09',13,26,6),
('CorrecciÃ³n de factura errÃ³nea','CORRECCIÃ“N FACTURA','2025-10-10',14,27,6),
('RevisiÃ³n de producto abierto','VERIFICACIÃ“N','2025-10-11',15,28,6),
('RevisiÃ³n de entrega incompleta','VERIFICACIÃ“N','2025-12-13',16,36,6);

SELECT 1;

SELECT 'Inicializacion staging completa (paridad seeds repo)' AS resultado;
