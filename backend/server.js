...
/* ────────────────────────────
   ...
   - Permite Google Maps en local
   - Mantiene jsDelivr, cdnjs y kit de FontAwesome
────────────────────────────── */
app.use((req, res, next) => {
  const csp = [
    "default-src 'self' http://localhost:3000",
    // JS externos permitidos (incluye Maps)
    ...
    // Estilos (Bootstrap/FA/Google Fonts) con inline para Bootstrap
...
app.use("/api/orders", orderRoutes); // HISTORIAL DE PEDIDOS
...
app.get("/api/orders/stream", (req, res) => {
  try {
    ...
    const userId = payload.id_usuario || payload.user?.id_usuario || payload.id || payload.sub; // por si tu JWT tiene otra clave
...
/* ────────────────────────────
   404
────────────────────────────── */
app.use("*", (_req, res) => res.status(404).sendFile(path.join(VIEWS_DIR, "404.html")));
